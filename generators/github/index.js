...
module.exports = class extends Generator {
  ...
  writing() {
    ...
    // write other file
    const that = this;
    copyFiles('.editorconfig', that);
    copyFiles('.eslintrc.js', that);
    copyFiles('.gitignore', that);
    copyFiles('babel.config.js', that);
    copyFiles('commitlint.config.js', that);
    copyFiles('license', that);
    this.fs.copy(
      this.templatePath('.vscode'),
      this.destinationPath('.vscode')
    );
    copyFiles('build', that);
    // copyFiles("src",that);
    copyFiles('tool', that);
  }
};
